...
class TestLayoutToString:
    ...
    def test_round_trip_cramped_room(self):
        ...
        reconstructed = Layout.from_string(layout_str, possible_recipes=original.possible_recipes)
    ...
    def test_round_trip_player_conveyor(self):
        ...
        assert len(reconstructed.player_conveyor_info) == len(original.player_conveyor_info)
        assert set(reconstructed.player_conveyor_info) == set(original.player_conveyor_info)
    ...
    def test_round_trip_all_registered_layouts(self):
        ...
        for layout_name, original in overcooked_v3_layouts.items():
            ...
            assert len(reconstructed.agent_positions) == len(original.agent_positions), f"Failed for {layout_name}"
            assert (reconstructed.static_objects == original.static_objects).all(), f"Failed for {layout_name}"
...
class TestValidateLayout:
    ...
    def test_missing_agents(self):
        """Test that layout without agents fails validation."""
        layout_str = """
WWPWW
0   0
W   W
...
"""
        ...
        assert not is_valid
        assert any("agent" in msg.lower() for msg in messages)
...
"""
        ...
        assert not is_valid
        assert any("goal" in msg.lower() or "delivery" in msg.lower() for msg in messages)
