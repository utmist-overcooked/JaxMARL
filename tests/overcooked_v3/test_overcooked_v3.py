...
class TestOvercookedV3RewardShaping:
    """Test shaped rewards for plate and soup interactions."""
...
""",
    ...
    def test_plate_pickup_rewarded_for_full_cooking_pot(self):
        env, state = self._make_env_and_state()
    ...
    def test_soup_in_dish_reward_for_cooked_soup_pickup(self):
        env, state = self._make_env_and_state()
        pot_contents = (DynamicObject.ingredient(0) * 3) | DynamicObject.COOKED
        state = self._set_pot(state, pot_contents, timer=4)
        state = state.replace(
            agents=state.agents.replace(
                pos=Position(
                ...
                dir=jnp.array([Direction.RIGHT], dtype=jnp.int32),
                inventory=jnp.array([DynamicObject.PLATE], dtype=jnp.int32),
            )
        )

        actions = {"agent_0": int(Actions.interact)}
        _, new_state, _, _, info = env.step(jax.random.PRNGKey(1), state, actions)

        assert float(info["shaped_reward"]["agent_0"]) == pytest.approx(5.0)
        assert new_state.agents.inventory[0] == (
