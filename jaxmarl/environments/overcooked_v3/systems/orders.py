...
def process_order_queue(
    ...
    # Generate either a weighted random recipe or a deterministic onion/tomato
    ...
    if config.order_queue_mode == "alternating":
        ...
        if config.recipe_probs is None:
            recipe_idx = jax.random.randint(
                subkey, (), 0, config.possible_recipes.shape[0]
            )
        else:
            recipe_idx = jax.random.choice(
                subkey,
                config.possible_recipes.shape[0],
                p=config.recipe_probs,
            )
        # Zero denotes an empty queue slot; recipe indices are one-based.
        new_order_type = recipe_idx + 1
