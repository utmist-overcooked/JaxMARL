...
def _load_jaxmarl_deps():
    ...
    global layout_to_string, get_layout_info, validate_layout, annotate_layout_string, OvercookedV3Visualizer
    
    if DEPENDENCIES_AVAILABLE:
        return
    
    try:
        from jaxmarl.environments.overcooked_v3.common import StaticObject as SO, Direction as Dir
        from jaxmarl.environments.overcooked_v3.layouts import Layout as L, overcooked_v3_layouts as layouts
...
TOOLS = [
    EditorTool("Wall", "W", "Blocks movement and acts as a counter.", COLOR_GRAY, StaticObject.WALL if DEPENDENCIES_AVAILABLE else FALLBACK_WALL, keyboard_shortcut='w'),
    EditorTool("Pot", "P", "Cooks ingredients into dishes.", COLOR_ORANGE, StaticObject.POT if DEPENDENCIES_AVAILABLE else FALLBACK_POT, keyboard_shortcut='p'),
    EditorTool("Plate Pile", "B", "Provides clean plates for serving.", COLOR_WHITE, StaticObject.PLATE_PILE if DEPENDENCIES_AVAILABLE else FALLBACK_PLATE_PILE, keyboard_shortcut='b'),
    EditorTool("Delivery", "X", "Deliver completed dishes here.", COLOR_GREEN, StaticObject.GOAL if DEPENDENCIES_AVAILABLE else FALLBACK_GOAL, keyboard_shortcut='x'),
    EditorTool("Recipe", "R", "Shows the current recipe to cook.", COLOR_PURPLE, StaticObject.RECIPE_INDICATOR if DEPENDENCIES_AVAILABLE else FALLBACK_RECIPE, keyboard_shortcut='r'),
    EditorTool("Agent", "A", "Sets an agent spawn position.", COLOR_BLUE, is_agent=True, keyboard_shortcut='a'),
    EditorTool("Ingredient 0", "0", "Onion pile (ingredient source).", COLOR_YELLOW, StaticObject.INGREDIENT_PILE_BASE if DEPENDENCIES_AVAILABLE else FALLBACK_INGREDIENT_BASE, keyboard_shortcut='0'),
    EditorTool("Ingredient 1", "1", "Tomato pile (ingredient source).", COLOR_RED, (StaticObject.INGREDIENT_PILE_BASE + 1) if DEPENDENCIES_AVAILABLE else (FALLBACK_INGREDIENT_BASE + 1), keyboard_shortcut='1'),
    EditorTool("Ingredient 2", "2", "Lettuce pile (ingredient source).", (0, 150, 0), (StaticObject.INGREDIENT_PILE_BASE + 2) if DEPENDENCIES_AVAILABLE else (FALLBACK_INGREDIENT_BASE + 2), keyboard_shortcut='2'),
    EditorTool("Item Conv >", ">", "Moves items to the right.", COLOR_CYAN, conveyor_direction=0 if DEPENDENCIES_AVAILABLE else 0),
    EditorTool("Item Conv <", "<", "Moves items to the left.", COLOR_CYAN, conveyor_direction=2 if DEPENDENCIES_AVAILABLE else 2),
    EditorTool("Item Conv ^", "^", "Moves items upward.", COLOR_CYAN, conveyor_direction=3 if DEPENDENCIES_AVAILABLE else 3),
    EditorTool("Item Conv v", "v", "Moves items downward.", COLOR_CYAN, conveyor_direction=1 if DEPENDENCIES_AVAILABLE else 1),
    EditorTool("Player Conv ]", "]", "Moves agents to the right.", COLOR_PURPLE, conveyor_direction=0 if DEPENDENCIES_AVAILABLE else 0),
...
SHORTCUT_TO_TOOL = {tool.keyboard_shortcut: i for i, tool in enumerate(TOOLS) if tool.keyboard_shortcut}
...
class EditorState:
    ...
    static_objects: np.ndarray = field(default_factory=lambda: np.zeros((DEFAULT_GRID_HEIGHT, DEFAULT_GRID_WIDTH), dtype=int))
    agent_positions: List[Tuple[int, int]] = field(default_factory=list)
    item_conveyors: Dict[Tuple[int, int], int] = field(default_factory=dict)  # (y,x) -> direction
    player_conveyors: Dict[Tuple[int, int], int] = field(default_factory=dict)  # (y,x) -> direction
...
class LevelEditor:
    ...
    def __init__(self):
        ...
        self.grid_width = DEFAULT_GRID_WIDTH * TILE_SIZE
        self.grid_height = DEFAULT_GRID_HEIGHT * TILE_SIZE
        self.window_width = TOOLBAR_WIDTH + self.grid_width + INFO_PANEL_WIDTH
    ...
    def handle_mouse_click(self, button: int, pos: Tuple[int, int]):
        ...
        if 0 <= grid_x < self.state.width * TILE_SIZE and 0 <= grid_y < self.state.height * TILE_SIZE:
            cell_x = grid_x // TILE_SIZE
            cell_y = grid_y // TILE_SIZE
        ...
        if self.show_resize_dialog:
            if key == 'backspace':
                ...
                try:
                    ...
                    if MIN_GRID_WIDTH <= new_width <= MAX_GRID_WIDTH and MIN_GRID_HEIGHT <= new_height <= MAX_GRID_HEIGHT:
                        self.state.resize(new_width, new_height)
                        self.show_resize_dialog = False
                        
                        # Update window size
                        self.grid_width = new_width * TILE_SIZE
                        self.grid_height = new_height * TILE_SIZE
                        self._set_window_size(
                            TOOLBAR_WIDTH + self.grid_width + INFO_PANEL_WIDTH,
...
'''
    ...
    def _layout_string_from_state(self) -> str:
        ...
        item_symbols = {0: '>', 2: '<', 3: '^', 1: 'v'}
        player_symbols = {0: ']', 2: '[', 3: '{', 1: '}'}
...
'''
    ...
    def draw_toolbar(self):
        ...
        for i, tool in enumerate(TOOLS):
            ...
            if i == self.state.selected_tool:
                pygame.draw.rect(self.screen, COLOR_BLUE, (5, y, TOOLBAR_WIDTH - 10, tool_height - 2))
            else:
                pygame.draw.rect(self.screen, COLOR_GRAY, (5, y, TOOLBAR_WIDTH - 10, tool_height - 2))
            ...
            if tool.keyboard_shortcut:
                shortcut_text = self.small_font.render(f"[{tool.keyboard_shortcut}]", True, COLOR_LIGHT_GRAY)
    ...
    def draw_grid(self):
        ...
        grid_rect = pygame.Rect(grid_x_offset, grid_y_offset, self.state.width * TILE_SIZE, self.state.height * TILE_SIZE)
        ...
        for y in range(self.state.height):
            for x in range(self.state.width):
                cell_rect = pygame.Rect(
                    grid_x_offset + x * TILE_SIZE,
                    grid_y_offset + y * TILE_SIZE,
                    TILE_SIZE,
                    TILE_SIZE
                )
        ...
        if self.hover_pos:
            ...
            if 0 <= grid_x < self.state.width * TILE_SIZE and 0 <= grid_y < self.state.height * TILE_SIZE:
                ...
                hover_rect = pygame.Rect(
                    grid_x_offset + cell_x * TILE_SIZE,
                    grid_y_offset + cell_y * TILE_SIZE,
                    TILE_SIZE,
                    TILE_SIZE
                )
    ...
    def _draw_plate_pile(self, rect: pygame.Rect):
        ...
        for x, y in plate_positions:
            # Plate circle
            pygame.draw.circle(self.screen, COLOR_WHITE, (int(x), int(y)), 
                             int(rect.width * 0.2))
            # Plate outline
            pygame.draw.circle(self.screen, COLOR_GRAY, (int(x), int(y)), 
                             int(rect.width * 0.2), 1)
    ...
    def _draw_item_conveyor(self, rect: pygame.Rect, direction: int = 0):
        ...
        for i in range(4):
            ...
            pygame.draw.line(self.screen, COLOR_GRAY,
                           ...
                           (rect.x + int(rect.width * 0.95), y_offset), 2)
    ...
    def draw_info_panel(self):
        ...
        panel_x = TOOLBAR_WIDTH + self.state.width * TILE_SIZE
        pygame.draw.rect(self.screen, COLOR_DARK_GRAY, (panel_x, TOP_MENU_HEIGHT, INFO_PANEL_WIDTH, self.window_height))
            ...
            for line in lines:
                ...
                for word in words:
                    if len(current_line + word) <= max_chars:
                        ...
                        text = self.small_font.render(current_line.rstrip(), True, COLOR_WHITE)
                        self.screen.blit(text, (x, current_y))
                        current_y += 22
                        current_line = word + " "
                if current_line:
    ...
    def draw_export_dialog(self):
        ...
        pygame.draw.rect(self.screen, COLOR_WHITE, (dialog_x, dialog_y, dialog_width, dialog_height))
        pygame.draw.rect(self.screen, COLOR_BLUE, (dialog_x, dialog_y, dialog_width, dialog_height), 3)
        ...
        # Draw close button
        close_button = pygame.Rect(dialog_x + dialog_width - 100, dialog_y + dialog_height - 50, 80, 35)
    ...
    def draw_resize_dialog(self):
        ...
        self.screen.blit(height_text, (height_input_rect.x + 10, height_input_rect.y + 5))
        ...
        cancel_button = pygame.Rect(dialog_x + 240, dialog_y + dialog_height - 50, 80, 35)
            ...
            # OK button
                ...
                try:
                    ...
                    if not (MIN_GRID_WIDTH <= new_width <= MAX_GRID_WIDTH):
                        self.resize_error = f"Width must be {MIN_GRID_WIDTH}-{MAX_GRID_WIDTH}"
                        return
                    
                    if not (MIN_GRID_HEIGHT <= new_height <= MAX_GRID_HEIGHT):
                        self.resize_error = f"Height must be {MIN_GRID_HEIGHT}-{MAX_GRID_HEIGHT}"
                        return
    ...
    def load_layout(self, layout_name: str):
        ...
        try:
            ...
            new_state.recipes = layout.possible_recipes.copy() if layout.possible_recipes else [[0, 0, 0]]
            ...
            # Update window size to fit new layout
            self.grid_width = new_state.width * TILE_SIZE
            self.grid_height = new_state.height * TILE_SIZE
            self._set_window_size(
                TOOLBAR_WIDTH + self.grid_width + INFO_PANEL_WIDTH,
                TOP_MENU_HEIGHT + self.grid_height
            )
