...
class OvercookedV3Visualizer:
    ...
    def _render_cell(
        ...
        def _render_pot(cell, img):
            return OvercookedV3Visualizer._render_pot(
                cell, img, pot_cook_time, pot_burn_time, pot_cook_duration
            )
    ...
    def _render_pot(
        ...
        """Render one pot tile, including its cook/burn progress bar.

        pot_cook_duration is the duration actually sampled for this pot (0 when
        the pot is idle); it falls back to pot_cook_time so fixed-cook-time
        layouts render as before.
        """
        if pot_cook_duration is None:
            pot_cook_duration = pot_cook_time
