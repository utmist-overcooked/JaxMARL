    ...
    def train(rng):
            ...
            if config.get("LOG_AGENTS_SEPARATELY", False):
                for i, a in enumerate(env.agents):
                    m = jax.tree.map(
                        lambda x: (
                            x[..., i].mean()
                            if x.ndim > 0 and x.shape[-1] == len(env.agents)
                            else x.mean()
                        ),
                        infos,
                    )
        ...
        def get_greedy_metrics(rng, train_state):
            ...
            if config.get("LOG_AGENTS_SEPARATELY", False):
                metrics = {}
                for i, a in enumerate(env.agents):
                    returned_episode = infos["returned_episode"][..., i]

                    def _agent_or_global_metric(x):
                        if x.ndim > 0 and x.shape[-1] == len(env.agents):
                            x = x[..., i]
