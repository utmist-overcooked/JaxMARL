...
def make_train(args):
    # Overcooked environment options passed via command line kwargs
    env_kwargs = {
        "layout": args.layout,
    ...
    def train_step(state, env_state, last_obs, last_dones, rnn_carry, rng):
        ...
        def step_fn(carry, _):
            ...
            # Extract shaped updates safely if active in your wrapper layers
            ...
            if isinstance(shaped_info, dict) and len(shaped_info) > 0:
                ...
                shaped_rews = jnp.zeros((args.num_envs, num_agents)) # Match shape [Batch, Agents]
            ...
            trans = Transition(
                ...
                shaped_rewards=shaped_rews # 💥 NEW: Store it here
            )
            
            # CRITICAL: Return next_dones_carry here to match input pytree structure
        ...
        def loss_fn(params):
            ...
            # rew_batch contains the total rewards collected during the step_fn execution
            ...
            # Value baseline targets calculated via central kitchen metrics
            ...
            # Normalize joint values over training updates to secure communication scales
            ...
            # --- CLEAN METRICS TRACKING VIA NAMEDTUPLE ATOMIC LOADING ---
            # traj.shaped_rewards shape: [Time, Batch, Agents]
            scaled_shaped_rews = traj.shaped_rewards * args.shaping_coef
            
            # Raw rewards are total collected rewards minus the dense shaping addition
...
def generate_validation_video(state, params, env_kwargs, max_steps, actor_module, num_envs):
    """Executes parallel validation steps to preserve Flax dense layer shapes while logging frame 0."""
    from jaxmarl import make as jaxmarl_make
    
    val_env = jaxmarl_make("overcooked_v3", **env_kwargs)
    viz = OvercookedV3Visualizer(val_env)
    
    # Setup batched tracks matching training metrics exactly
    ...
    for _ in range(max_steps):
        # Unbatch the state of environment 0 to generate clean RGB frame matrices
        single_state = jax.tree_util.tree_map(lambda x: x[0], env_state)
        frame = viz.render_state(single_state)
        frames.append(frame)
        
        # Build tensor alignments matching standard training shapes [B, N, ...]
        obs_tensor = jnp.stack([obs[a] for a in val_env.agents], axis=1) # [B, N, H, W, C]
        dones_tensor = jnp.stack([dones[a] for a in val_env.agents], axis=1)[..., None] # [B, N, 1]
        ...
        # Forward pass down the compiled sequence network paths
        _, (logits_seq, _, _) = actor_module.apply(
            params['actor'], rnn_masked, obs_seq, dones_seq
        )
        logits = logits_seq[0] # [B, N, ActDim]
        
        # Extract greedy action selections across the entire environment batch
    ...
    return frames # Clean list of un-transposed arrays
...
def main():
    ...
    parser.add_argument("--update_timestep", type=int, default=32) 
    ...
    train_state = AgentState.create(
        ...
        tx=optax.chain(
            # Switch from global norm to absolute element-wise clipping 
            # to prevent single massive logit updates from dominating
            optax.clip(0.1), 
            optax.rmsprop(
                learning_rate=lr_schedule, 
                decay=args.alpha,
                eps=1e-5  # Prevents division by near-zero variance
            ) 
        ),
        critic_net=critic
    )
    ...
    for update in range(1, total_updates + 1):
        ...
        if update % 10 == 0:
            ...
            print(f"Update {update}/{total_updates} | SPS: {sps:.0f} | Total Rew: {metrics['rew/total_step_reward']:.3f}")
            ...
            action_names = ["move_right", "move_down", "move_left", "move_up", "stay", "interact"]
            for name in action_names:
                key = f"actions/{name}"
                if key in metrics:
                    wandb_metrics[f"actions/{name}_percentage"] = metrics[key]

            wandb.log(wandb_metrics)
            start_time = time.time()
            
        # Drop an atomic weights checkpoint every 5000 updates
        if update % 5000 == 0:
            import os
            import pickle
        ...
        # 🎥 Fast GIF compilation file path trigger every 500 updates
        # if update % 500 == 0:
        ...
        #     # Compress list of arrays into a standard local file on disk
        #     local_gif_path = "current_kitchen_rollout.gif"
        #     imageio.mimsave(local_gif_path, gif_frames, fps=8)
            
        #     # HARDENED FIX: Use wandb.Image to reference the local filepath string.
        #     # This completely avoids moviepy checks and displays perfectly on your workspace dashboard.
        #     wandb.log({
        #         "media/kitchen_coordination_loop": wandb.Image(local_gif_path),
        #         "global_step": update * args.num_envs * args.update_timestep
        #     })
