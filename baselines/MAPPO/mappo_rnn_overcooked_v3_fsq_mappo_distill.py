...
and actor distillation from a privileged full-observation MAPPO teacher.

The teacher is a trained full-observation MAPPO actor (baselines/MAPPO/
mappo_rnn_overcooked_v3_full_obs.py::ActorRNN) trained on the full grid
observation (agent_view_size=None). The student sees only a partial view
(agent_view_size) and must learn an FSQ communication channel to recover the
teacher's behaviour. The teacher is a policy, so its action logits are used
directly for the KL distillation (no critic; no Q-standardization needed).

The teacher actor shares this file's CNN / ScannedRNN definitions, so its
checkpoint (VmapCNN_0, LayerNorm_0, ScannedRNN_0 manual-GRU params, Dense_0/1)
loads directly into TeacherActorRNN below.
"""
...
def load_mappo_teacher_params(path):
    """Load a trained full-obs MAPPO actor (..._vmap0_actor.safetensors).

    `path` is the actor safetensors file, or a directory containing exactly one
    ..._actor.safetensors. mappo_rnn_overcooked_v3_full_obs.py saves the actor
    TrainState params as {"params": {...}} via save_params, which is exactly
    what TeacherActorRNN.apply expects.
    """
    if os.path.isdir(path):
        cands = [
            f
            for f in os.listdir(path)
            if f.endswith("_actor.safetensors")
        ]
        if len(cands) != 1:
            raise ValueError(
                f"Expected exactly one *_actor.safetensors in {path}, found {cands}"
            )
...
class TeacherActorRNN(nn.Module):
    """Privileged full-obs MAPPO teacher actor.

    Verbatim copy of mappo_rnn_overcooked_v3_full_obs.py::ActorRNN, reusing this
    file's CNN + ScannedRNN, so a trained full-obs MAPPO actor checkpoint loads
    with matching parameter names (VmapCNN_0, LayerNorm_0, ScannedRNN_0 with the
    manual-GRU params gru_Wi_*/gru_Wh_*/gru_b_*, Dense_0, Dense_1). Frozen; only
    its action logits `pi` are used for the KL distillation.
    """
...
def make_train(config, monitor=None, teacher_actor_params=None):
    ...
    # Teacher network dims must match the trained MAPPO teacher checkpoint,
    # independent of the student dims (defaults are the full_obs config: 128/64).
    teacher_config = {
        "GRU_HIDDEN_DIM": config.get("TEACHER_GRU_HIDDEN_DIM", 128),
        "FC_DIM_SIZE": config.get("TEACHER_FC_DIM_SIZE", 64),
    ...
    def train(rng):
        ...
        # MAPPO teacher shares this file's ScannedRNN (manual GRU); zeros carry.
...
def single_run(config):
    ...
    if not teacher_actor_path:
        raise ValueError(
            "TEACHER_ACTOR_PATH must point to a trained full-obs MAPPO actor "
            "(a ..._actor.safetensors file, or a dir containing exactly one)."
        )
    teacher_actor_params = load_mappo_teacher_params(teacher_actor_path)
    print(f"[distill] MAPPO teacher loaded from {teacher_actor_path}", flush=True)
    ...
    wandb.init(
        ...
        tags=["MAPPO", "RNN", "OvercookedV3", "FSQComm", "Distill", "MAPPOTeacher"],
        config=copy.deepcopy(config),
        mode=config["WANDB_MODE"],
        name=config["WANDB_RUN_NAME"]
        or f"mappo_rnn_overcooked_v3_fsq_mappo_distill_{layout_name}",
    )
    ...
    OmegaConf.save(
        ...
        os.path.join(
            save_dir,
            f"mappo_rnn_overcooked_v3_fsq_mappo_distill_{layout_name}_seed{config['SEED']}_config.yaml",
        ),
    )
    ...
    for i, rng in enumerate(rngs):
        ...
        actor_path = os.path.join(
            save_dir,
            f"mappo_rnn_overcooked_v3_fsq_mappo_distill_{layout_name}_seed{config['SEED']}_vmap{i}_actor.safetensors",
        )
        critic_path = os.path.join(
            save_dir,
            f"mappo_rnn_overcooked_v3_fsq_mappo_distill_{layout_name}_seed{config['SEED']}_vmap{i}_critic.safetensors",
        )
...
@hydra.main(
    version_base=None,
    config_path="config",
    config_name="mappo_rnn_overcooked_v3_fsq_mappo_distill",
)
def main(config):
