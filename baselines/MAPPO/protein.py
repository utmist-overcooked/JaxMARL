"""Protein hyperparameter sweep optimizer (vendored from PufferLib).

This file is a near-verbatim copy of PufferLib's `pufferlib/sweep.py` at tag 4.0:
    https://github.com/PufferAI/PufferLib/blob/4.0/pufferlib/sweep.py
Copyright (c) PufferAI, released under the MIT License. Vendored here so the
Overcooked V3 macro-action baselines can run the Protein optimizer without
taking a dependency on the full PufferLib package.

The ONLY change from upstream is the removal of the unused `import pufferlib`
line (the module never references the `pufferlib` namespace). Everything else --
the Space classes, `Hyperparameters`, the Pareto helpers, `RobustLogCostModel`,
the `Random`/`ParetoGenetic` baselines, and the `Protein` optimizer itself -- is
kept as-is so behaviour matches the reference implementation.

`Protein` maintains two Gaussian Processes (one over score, one over log-cost),
generates candidates around the Pareto frontier of past observations, and picks
the candidate that maximises predicted score subject to a cost budget. The driver
in `protein_sweep.py` wraps it around the shared `run_experiment(...)` entry point.

Optional dependencies (installable via `pip install -e '.[sweep]'`):
    torch, gpytorch, scipy, scikit-learn
"""
