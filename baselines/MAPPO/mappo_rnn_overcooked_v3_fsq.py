"""
MAPPO RNN for Overcooked V3 with FSQ communication.

Actors see partial observations and exchange quantized (finite scalar
quantized) messages: each agent encodes its recurrent state into a discrete
code and reads its partner's code before choosing an action. The critic is
centralized over the concatenated partial observations.

Ported from mappo_rnn_overcooked_v3_fsq.py with the full-observation
teacher distillation removed - this trains the communicating policy from
scratch on the environment reward alone.
"""
...
# baselines/MAPPO for the sibling fsq modules, then the repo root so `jaxmarl`
# resolves to this checkout rather than any copy installed in site-packages.
...
COMM_TYPES = ("fsq", "commnet", "none")


def resolve_comm_type(config) -> str:
    """Which inter-agent channel to use: 'fsq', 'commnet' or 'none'.

    DISABLE_FSQ_COMM is the older switch and still wins when set, so existing
    configs and ablation runs keep behaving exactly as before.
    """
    if config.get("DISABLE_FSQ_COMM", False):
        return "none"
    comm_type = str(config.get("COMM_TYPE", "fsq")).lower()
    if comm_type not in COMM_TYPES:
        raise ValueError(
            f"COMM_TYPE must be one of {COMM_TYPES}, got {comm_type!r}"
        )
    return comm_type


class ScannedRNN(nn.Module):
    """GRU over the time axis, driven by jax.lax.scan.

    Same interface as the flax ``nn.scan`` version this replaces - call it with
    ``(carry, (inputs, resets))`` where inputs is ``(T, batch, features)`` - but
    the recurrent weights are raw params and the input projections are computed
    for every timestep up front, outside the scan. flax's ``nn.scan`` reaches
    for ``jax.api_util.debug_info``, which the pinned jax does not provide, so
    it cannot be used in this environment. This mirrors the GRU that the
    working IPPO Overcooked V3 baseline uses.
    """

    @nn.compact
    def __call__(self, carry, x):
        ins, resets = x
        time_steps, batch_size, hidden_dim = ins.shape

        # Input projections for the whole sequence in one matmul each.
        flat = ins.reshape(-1, hidden_dim)
        proj = {}
        for gate in ("z", "r", "h"):
            proj[gate] = nn.Dense(
                hidden_dim, use_bias=False, name=f"gru_Wi_{gate}"
            )(flat).reshape(time_steps, batch_size, hidden_dim)

        Wh_z = self.param(
            "gru_Wh_z", nn.initializers.orthogonal(), (hidden_dim, hidden_dim)
        )
        Wh_r = self.param(
            "gru_Wh_r", nn.initializers.orthogonal(), (hidden_dim, hidden_dim)
        )
        Wh_h = self.param(
            "gru_Wh_h", nn.initializers.orthogonal(), (hidden_dim, hidden_dim)
        )
        ...
        def _gru_step(h, inp):
            wi_z, wi_r, wi_h, done = inp
            # Episode boundaries reset the hidden state, as in the original.
            h = jnp.where(done[:, np.newaxis], jnp.zeros_like(h), h)
            z = jax.nn.sigmoid(wi_z + h @ Wh_z + b_z)
            r = jax.nn.sigmoid(wi_r + h @ Wh_r + b_r)
            h_hat = jnp.tanh(wi_h + (r * h) @ Wh_h + b_h)
            new_h = (1 - z) * h + z * h_hat
            return new_h, new_h

        return jax.lax.scan(
            _gru_step, carry, (proj["z"], proj["r"], proj["h"], resets)
        )
...
class CommActorRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        comm_type = resolve_comm_type(self.config)
        ...
        if comm_type == "none":
            ...
            # CommNet (Sukhbaatar et al. 2016): agents broadcast their hidden
            # state and each reads the MEAN of the others', folded back in over
            # COMMNET_ROUNDS rounds of  h <- act(H h + C c).  Continuous and
            # differentiable end to end, in contrast to the discrete FSQ codes.
            rounds = int(self.config.get("COMMNET_ROUNDS", 1))
            hidden_dim = embedding.shape[-1]
            state = embedding
            comm_vec = jnp.zeros_like(state)
            for r in range(rounds):
                by_agent = state.reshape(time_steps, num_agents, num_envs, hidden_dim)
                if num_agents > 1:
                    total = jnp.sum(by_agent, axis=1, keepdims=True)
                    others_mean = (total - by_agent) / (num_agents - 1)
                else:
                    others_mean = jnp.zeros_like(by_agent)
                comm_vec = others_mean.reshape(time_steps, batch_size, hidden_dim)
                ...
                state = nn.tanh(h_term + c_term)

            # Report the received message so the rest of the pipeline keeps a
            # uniform interface; there is no codebook, so indices stay 0.
            msg_codes = comm_vec
            msg_indices = jnp.zeros((time_steps, batch_size), dtype=jnp.int32)
            actor_input = state
...
def render_checkpoint_gif(
    *,
    ...
    fsq_enabled = (
        ...
        and resolve_comm_type(config) == "fsq"
...
def make_train(config, monitor=None):
    ...
    if env.num_agents != 2:
        # The partner message is formed by flipping the agent axis, which only
        # defines a unique partner for two agents.
        raise ValueError("FSQ communication currently supports 2 agents.")
    ...
    comm_type = resolve_comm_type(config)
    fsq = FSQ(levels=tuple(config["FSQ_LEVELS"]))
    ...
    def fsq_code_metrics(comm_index, comm_code):
        # Codebook statistics only mean something for the discrete FSQ channel.
        # CommNet messages are continuous and have no codebook, so report zeros
        # rather than a misleading "1 unique code".
        if comm_type != "fsq":
            return (
                jnp.zeros((fsq.codebook_size,), dtype=jnp.int32),
                jnp.zeros((len(config["FSQ_LEVELS"]), fsq_max_level), dtype=jnp.int32),
...
def single_run(config):
    ...
    wandb.init(
        ...
        or f"mappo_rnn_overcooked_v3_fsq_{layout_name}",
    )
    ...
    OmegaConf.save(
        ...
        os.path.join(
            save_dir,
            f"mappo_rnn_overcooked_v3_fsq_{layout_name}_seed{config['SEED']}_config.yaml",
        ),
    )
    ...
    for i, rng in enumerate(rngs):
        ...
        actor_path = os.path.join(
            save_dir,
            f"mappo_rnn_overcooked_v3_fsq_{layout_name}_seed{config['SEED']}_vmap{i}_actor.safetensors",
        )
        critic_path = os.path.join(
            save_dir,
            f"mappo_rnn_overcooked_v3_fsq_{layout_name}_seed{config['SEED']}_vmap{i}_critic.safetensors",
        )
...
@hydra.main(
    version_base=None,
    config_path="config",
    config_name="mappo_rnn_overcooked_v3_fsq",
)
def main(config):
