"""MAPPO RNN baseline for Overcooked V3.
...
def unbatchify(x: jnp.ndarray, agent_list, num_envs, num_actors):
    x = x.reshape((num_actors, num_envs) + x.shape[1:])
...
def rollout_seed_key(config):
    num_seeds = int(config.get("NUM_SEEDS", 1))
    seed_index = int(config.get("ROLLOUT_GIF_SEED_INDEX", 0))
    seed_index = min(max(seed_index, 0), num_seeds - 1)
    rngs = jax.random.split(jax.random.PRNGKey(int(config["SEED"])), num_seeds)
    return int(rngs[seed_index][0]), seed_index


def select_rollout_seed_params(params, original_seed, seed_index):
    seed_values = np.asarray(original_seed).reshape(-1)
    if seed_values.size <= 1:
        return params, int(seed_values[0])

    selected_index = min(seed_index, seed_values.size - 1)
    selected_params = jax.tree.map(
        lambda x: x[selected_index]
        if hasattr(x, "shape") and len(x.shape) > 0 and x.shape[0] == seed_values.size
        else x,
        params,
    )
...
def save_greedy_rollout_gif(actor_params, config, update_step, env_step, seed, run_name):
    ...
    rollout_seed = int(config["SEED"]) + int(update_step)
    ...
    for _ in range(max_steps):
        ...
        obs, state, _, done, _ = env.step(step_rng, state, env_act)
        states.append(state)

        done_all = bool(jax.device_get(done["__all__"]))
        done_batch = jnp.full((env.num_agents,), done_all, dtype=bool)
        if done_all:
            break

    gif_path = rollout_gif_path(
        config.get("ROLLOUT_GIF_DIR", "rollouts"),
        run_name,
        update_step,
        env_step,
        seed,
    ...
    viz.animate(stack_state_sequence(states), filename=str(gif_path))
    ...
    upload_msg = " and uploaded to wandb" if uploaded else ""
    print(f"Saved rollout GIF{upload_msg}: {gif_path}")
    return gif_path
...
def make_train(config):
    ...
    rollout_updates = rollout_milestones(
        config["NUM_UPDATES"],
        config.get("ROLLOUT_GIF_COUNT", 10),
    )
    rollout_enabled = bool(config.get("ROLLOUT_GIF_ENABLED", False)) and bool(
        rollout_updates
    )
    rollout_update_array = jnp.array(rollout_updates, dtype=jnp.int32)
    rollout_target_seed, rollout_seed_index = rollout_seed_key(config)
    ...
    def train(rng):
        ...
        def _update_step(update_runner_state, unused):
            ...
            if rollout_enabled:
                def rollout_callback(metrics, original_seed, actor_params):
                    try:
                        selected_params, seed = select_rollout_seed_params(
                            actor_params,
                            original_seed,
                            rollout_seed_index,
                        )
                        if seed != rollout_target_seed:
                            return

                        step = int(np.asarray(metrics["update_steps"]).reshape(-1)[0])
                        env_step = int(np.asarray(metrics["env_step"]).reshape(-1)[0])
                        run_name = config.get("RUN_NAME") or config.get("WANDB_NAME")
                        if config.get("WANDB_MODE") != "disabled" and wandb.run is not None:
                            run_name = wandb.run.name
                        save_greedy_rollout_gif(
                            selected_params,
                            config,
                            step,
                            env_step,
                ...
                is_rollout_update = jnp.any(update_steps == rollout_update_array)
                jax.lax.cond(
                    is_rollout_update,
                    record_rollout,
                    skip_rollout,
...
@hydra.main(version_base=None, config_path="config", config_name="mappo_homogenous_rnn_overcooked_v3")
