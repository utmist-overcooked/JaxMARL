    ...
    *,
    ...
    for i in range(num_agents):
        ...
        def actor_loss_fn(a_params, agent_i=i, key=subkey):
            ...
            cur_msgs_list    = []
            rng_inner = key
            logits_i  = None   # agent i's raw logits, kept for regularization

            for j in range(num_agents):
                rng_inner, sk = jax.random.split(rng_inner)
                logits_j, a_oh_j, m_j, _ = actor.apply(
                    a_params,
                    batch.obs[:, j, :],
                    received[:, j, :, :],
                    rng=sk,
            ...
            # Logit L2 regularization (as in the original MADDPG paper): without
            # it, -Q gradients through the straight-through Gumbel push one
            # action's logit unboundedly (we observed |logits| ~ 36 -> policy
            # delta-collapses to a single action and never recovers).
            logit_reg_loss = jnp.mean(logits_i ** 2)
            total_loss = q_loss + pred_loss_coef * pred_loss + logit_reg * logit_reg_loss
