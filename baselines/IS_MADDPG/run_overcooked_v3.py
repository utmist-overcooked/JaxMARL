...
def make_overcooked_config(layout: str, args: argparse.Namespace, env_info: dict) -> dict:
    ...
    return {
        **DEFAULT_CONFIG,
        ...
        "MSG_DIM":          5,
        "HORIZON_H":        2,
        # HIDDEN_DIM / LR / GRAD_CLIP mirror the QMIX CTC handoff recipe
        # (HIDDEN_SIZE=256, LR=5e-5, MAX_GRAD_NORM=10) so a QMIX-vs-MADDPG
        # gap on CTC points at MADDPG itself, not a hyperparameter mismatch.
        "HIDDEN_DIM":       256, # mirror QMIX HIDDEN_SIZE=256 (was 128)
        "ACTOR_LR":         5e-5, # mirror QMIX LR=0.00005 (was 1e-3)
        "CRITIC_LR":        5e-5, # mirror QMIX LR=0.00005 (was 1e-3)
        "GAMMA":            0.99, # match QMIX/IPPO; 0.90 made 60-step cook->deliver chains invisible (0.9^60~0.002)
        # NOTE: target-update mechanism is architecturally different and NOT mirrored:
        # QMIX does a HARD copy every 10 rollouts (TAU=1.0); MADDPG does a soft update
        # every env step. Likewise MADDPG trains every env step (NUM_EPOCHS grad updates
        # per step) vs QMIX once per 400-step rollout, so NUM_EPOCHS is left at 2 — mirroring
        # it to QMIX's 8 would quadruple an already-much-larger gradient budget.
        "TAU":              0.01,
        "GRAD_CLIP":        10.0, # mirror QMIX MAX_GRAD_NORM=10 (was 0.5)
        "GUMBEL_TAU":       1.0,
        "GUMBEL_HARD":      True,
        "PRED_LOSS_COEF":   0.05,
        ...
        "BATCH_SIZE":       1024,        # larger minibatch (cf. reference MADDPG)
        "BUFFER_SIZE":      200_000,     # memory-bound on 16GB GPU (1M obs would be ~23GB)
        "LEARNING_STARTS":  10_000,      # mirror QMIX LEARNING_STARTS=10000 (was 5000)
        "UPDATE_EVERY":     1,
        "UPDATES_PER_STEP": 2,
        "NUM_EPOCHS":       2,           # per-env-step; NOT comparable to QMIX per-rollout epochs (see above)

        # ── Exploration ──────────────────────────────────────────────────
        # Mirror QMIX: EPS_FINISH=0.05, decayed over the first 20% of training
        # (QMIX EPS_DECAY=0.2). Overcooked is dense-reward so signal shows early.
        "EPSILON_START":    1.0,
        "EPSILON_END":      0.05, # mirror QMIX EPS_FINISH=0.05 (was 0.1)
        "EPSILON_DECAY":    int(args.total_timesteps * 0.2), # mirror QMIX EPS_DECAY=0.2 (was 0.75)

        # ── Logging / saving ─────────────────────────────────────────────
        "SEED":             args.seed,
        "LOG_EVERY":        1_000,
        "LOG_INTERVAL":     10_000,      # env steps per chunk → one wandb.log + print
        "CKPT_INTERVAL":    500_000,     # periodic checkpoint every 500k env steps (+ _final.pkl); 0 = final-only
        "TEST_INTERVAL":    0.05,
        "SAVE_PATH":        args.save_path,
        # Periodic GIF rendering. SAVE_GIF_PATH is a base .gif path; a greedy
        # rollout is rendered every GIF_EVERY_FRAC of training (plus a final one)
        # with the step count spliced into the filename. None = disabled.
        "SAVE_GIF_PATH":    args.save_gif_path,
        "GIF_EVERY_FRAC":   0.1,         # render 1 GIF every 1/10th of the run
...
def render_gif_rollout(
    ...
    *,
    ...
    """Greedy single-env rollout that records env states and saves a GIF.

    Called periodically during training (every GIF_EVERY_FRAC of the run) to
    snapshot policy behaviour, so failures like "cook-and-burn farm" or "never
    crosses the conveyor" are visible without waiting for the end. Mirrors
    evaluate()'s action selection but collects states and renders them with the
    OvercookedV3 visualizer (reused from eval.py).
    """
    num_agents = config["NUM_AGENTS"]
    obs_dim    = config["OBS_DIM"]
    msg_dim    = config["MSG_DIM"]
    ...
    max_steps  = getattr(env, "max_steps", config.get("MAX_STEPS", 400))
    deliveries = 0
    ...
    for _step in range(max_steps):
        ...
        rng, act_rng = jax.random.split(rng)
        logits_all, _ = select_actions_jit(
            train_state.actor_params, actor, jnp.array(obs_all),
            jnp.array(prev_msgs), num_agents, gumbel_tau, act_rng,
        )
        rng, eps_rng = jax.random.split(rng)
        actions_idx, _ = apply_epsilon_greedy(
            logits_all, eps_rng, epsilon, num_agents, 1, act_dim
        )
        acts_idx = np.array(actions_idx)                # (1, N)
        action_dict = {f"agent_{i}": int(acts_idx[0, i]) for i in range(num_agents)}

        rng, step_key = jax.random.split(rng)
        obs_dict, env_state, rewards_dict, dones_dict, _ = env.step_env(
            step_key, env_state, action_dict
        )
        state_seq.append(jax.tree_util.tree_map(lambda x: np.array(x).copy(), env_state))
        if sum(float(rewards_dict[aid]) for aid in agent_ids) >= 20.0:
            deliveries += 1
        if bool(dones_dict.get("__all__", False)):
            break

    try:
        from eval import save_episode_gif
        os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
        save_episode_gif(state_seq, env, path, fps=8)
        print(f"  [gif] {path}  ({len(state_seq)} frames, {deliveries} greedy deliveries)",
              flush=True)
    except Exception as e:
        print(f"  [gif] render failed ({type(e).__name__}: {e})", flush=True)
    ...
    def jit_train_step(state, batch):
        ...
        def epoch_step(carry, _):
            s, metrics = train_step(
                ...
                actor_lr=config["ACTOR_LR"],
                critic_lr=config["CRITIC_LR"],
                logit_reg=config.get("LOGIT_REG", 1e-3),
            )
            return s, metrics
...
# Version 2 (single lax.scan over total_steps_target) — more efficient than the original for loop over total_steps_target
    ...
    def _update_step(runner_state, step_idx):
        ...
        step_metrics = {
            ...
            # Full env event set (mean over envs each step), matching how QMIX/IPPO
            # log info["event/*"] so the same event charts appear.
            **{k: info[k].mean() for k in info
               if isinstance(k, str) and k.startswith("event/")},
        }
    ...
    # ── Chunked compiled scan ─────────────────────────────────────────
    # Instead of one giant lax.scan over all 375k iterations (no logging /
    # no checkpointing / no early-stop), scan a fixed CHUNK_LEN block at a
    # time inside a jitted fn, then drop to Python to log + checkpoint.
    # CHUNK_LEN is static so run_chunk compiles exactly once.
    chunk_len  = max(1, int(config["LOG_INTERVAL"]) // num_envs)
    num_chunks = max(1, total_steps_target // chunk_len)
    ckpt_interval = int(config.get("CKPT_INTERVAL", 0))   # 0 = final-only
    ckpt_every_chunks = (
        max(1, ckpt_interval // max(1, chunk_len * num_envs))
        if ckpt_interval > 0 else 0
    )

    # Periodic GIF rendering: one greedy rollout every GIF_EVERY_FRAC of the run
    # (default 1/10th) + a final one, so behaviour is watchable mid-training.
    gif_base = config.get("SAVE_GIF_PATH", None)
    gif_every_chunks = (
        max(1, int(round(num_chunks * config.get("GIF_EVERY_FRAC", 0.1))))
        if gif_base else 0
    )
    gif_rng = jax.random.PRNGKey(config["SEED"] + 12345)

    def _render_gif(global_step):
        nonlocal gif_rng
        gif_rng, r = jax.random.split(gif_rng)
        root, ext = os.path.splitext(gif_base)
        render_gif_rollout(
            runner_state.train_state, actor, env_vec, r,
            config=config,
            path=f"{root}_step{global_step:09d}{ext or '.gif'}",
            epsilon=0.0,
        )

    @jax.jit
    def run_chunk(rs, step_indices):
        return jax.lax.scan(_update_step, rs, step_indices, length=chunk_len)

    print(f"Chunked training: {num_chunks} chunks × {chunk_len} iters "
          f"({chunk_len*num_envs:,} env steps each). "
          f"First chunk includes JIT compile (several minutes)...", flush=True)

    def _summ(cm, ready):
        """Mean of a per-step metric over steps where an update ran."""
        return lambda k: (float(cm[k][ready].mean()) if ready.any() else 0.0)
    ...
    for c in range(num_chunks):
        t_c = time.time()
        start = c * chunk_len
        idx = jnp.arange(start, start + chunk_len)
        runner_state, cm = run_chunk(runner_state, idx)
        cm = jax.block_until_ready(cm)
        cm = {k: np.asarray(v) for k, v in cm.items()}   # → host, frees device
        chunk_metrics_host.append(cm)

        global_step = (start + chunk_len) * num_envs
        ready = cm["critic_loss"] != 0.0
        m = _summ(cm, ready)

        # completed-episode aggregates within this chunk
        ep_mask    = cm["completed_lengths"] > 0
        n_ep       = int(ep_mask.sum())
        ep_ret     = float(cm["completed_returns"][ep_mask].mean()) if n_ep else 0.0
        deliveries = int(cm["completed_delivery"][ep_mask].sum())   if n_ep else 0
        soup       = int(cm["completed_soup_in_dish"][ep_mask].sum()) if n_ep else 0
        mean_reward = float(cm["rewards"].mean())

        try:
            import wandb
            payload = {
                "env_step":            global_step,
                "train/step_reward":   mean_reward,
                "train/critic_loss":   m("critic_loss"),
                "train/actor_loss":    m("actor_loss"),
                "train/pred_loss":     m("pred_loss"),
                "train/q_mean":        m("q_mean"),
                "episode/return_mean": ep_ret,
                "episode/count":       n_ep,
            }
            # Full env event set (per-chunk mean), matching QMIX/IPPO event/* charts:
            # event/pot_placement, /pot_start_cooking, /dish_pickup, /delivery, /pot_burn,
            # /pickup, /drop, /dish_to_goal_progress, /order_added, /order_expired.
            for k in cm:
                if isinstance(k, str) and k.startswith("event/"):
                    payload[k] = float(np.asarray(cm[k]).mean())
            wandb.log(payload, step=global_step)
        except Exception:
            pass

        print(f"[chunk {c+1}/{num_chunks}] step={global_step:,} "
              f"reward={mean_reward:.3f} ep_ret={ep_ret:.2f} eps={n_ep} "
              f"deliv={deliveries} soup={soup} "
              f"closs={m('critic_loss'):.3f} aloss={m('actor_loss'):.3f} "
              f"({time.time()-t_c:.1f}s)", flush=True)

        if ckpt_dir is not None and ckpt_every_chunks and (c + 1) % ckpt_every_chunks == 0:
            save_checkpoint(
                runner_state.train_state,
                os.path.join(ckpt_dir, f"is_maddpg_{config['LAYOUT']}_step{global_step}.pkl"),
                config,
            )

        if gif_every_chunks and (c + 1) % gif_every_chunks == 0:
            _render_gif(global_step)

    # Stitch per-chunk metrics back into the (total_steps, ...) layout the
    # downstream plotting / CSV code expects from the old single-scan path.
    all_metrics = {
        k: np.concatenate([cm[k] for cm in chunk_metrics_host], axis=0)
        for k in chunk_metrics_host[0]
    }
    if ckpt_dir is not None:
        save_checkpoint(
            runner_state.train_state,
            os.path.join(ckpt_dir, f"is_maddpg_{config['LAYOUT']}_final.pkl"),
            config,
        )
    if gif_base:
        _render_gif((num_chunks * chunk_len) * num_envs)
    print(f"Training complete. {num_chunks} chunks took {time.time()-t_train:.1f}s", flush=True)
...
def main():
    ...
    parser.add_argument(
        "--save_gif_path", type=str, default=None,
        help="Base .gif path for periodic greedy-rollout GIFs (one every 1/10th "
             "of training + a final one). None to disable."
    )
    parser.add_argument(
    ...
    # Register env_step as the x-axis for every metric (matches QMIX/IPPO's
    # define_wandb_metrics) so all event/* panels are created and plotted
    # against env_step — including flat-zero series.
    ...
    #
    # CTC needs the same env config the QMIX scripts pass (alternating order
    # queue + conveyors) for a fair comparison. The argparse CLI can't carry
    # these, so set them here, keyed on the conveyor/handoff layouts.
    env_kwargs = dict(layout=args.layout, max_steps=args.max_steps)
    # The alternating order-queue config matches the proven IPPO/QMIX recipes (CTC + around
    # _the_island both use it) — without it the runs aren't comparable. Conveyor flags are
    # forced ONLY on conveyor maps; elsewhere (e.g. around_the_island) leave them unset so
    # OvercookedV3 auto-detects from the layout.
    if args.layout in ("coordinated_temporal_conveyor", "maze_conveyor_hell",
                       "around_the_island"):
        env_kwargs.update(
            pot_cook_time=60,
            pot_burn_time=90,
    ...
    env_vec  = OvercookedV3(**env_kwargs)
