#!/usr/bin/env bash
# MAPPO-RNN + FSQ communication (no distillation) on around_the_island.
#
# Agents are partially observed (agent_view_size=2) and exchange one discrete
# FSQ-quantized message per step: each encodes its recurrent state into a code
# from a 5x5x5 = 125-word codebook and reads its partner's code before acting.
...
NAME=mappo_fsq_comm_around_the_island_10m
...
# One GPU on this host: never run alongside the CommNet job.
if tmux has-session -t jaxmarl_commnet_comm_waiting 2>/dev/null; then
  echo "[$(date +%F_%T)] waiting for the CommNet run to finish first..." | tee -a "$LOG"
  while tmux has-session -t jaxmarl_commnet_comm_waiting 2>/dev/null; do sleep 60; done
fi
...
"$REPO/.venv/bin/python" "$REPO/baselines/MAPPO/mappo_rnn_overcooked_v3_fsq.py" \
  ...
  SEED=0 NUM_SEEDS=1 \
  WANDB_MODE=online PROJECT=ocv3_mappo_fsq_comm ENTITY=zacharytang24- \
  WANDB_RUN_NAME=$NAME \
  USE_RICH_MONITOR=False \
  DISABLE_CHECKPOINTS=False \
  CHECKPOINT_GIF=False CHECKPOINT_FSQ_VIEWER=True \
  >> "$LOG" 2>&1
