...
WANDB_PROJECT="${WANDB_PROJECT:-ippo_v3_cramped_room_${CURRENT_TIME}}"
SAVE_PATH="${SAVE_PATH:-$ROOT_DIR/checkpoints/ippo_v3_cramped_room_${CURRENT_TIME}}"

mkdir -p "$SAVE_PATH"

echo "Starting IPPO training for cramped_room (v3)"
...
"$PYTHON_BIN" "$ROOT_DIR/baselines/IPPO/ippo_rnn_overcooked_v3.py" \
  ...
  WANDB_NAME="ippo_v3_cramped_room" \
