...
ENTITY = os.getenv("WANDB_ENTITY", "zacharytang24-")
...
def _history_key_names(history_keys):
	if isinstance(history_keys, dict):
		keys = history_keys.get("keys", history_keys)
		if isinstance(keys, dict):
			return sorted(str(k) for k in keys)
		if isinstance(keys, (list, tuple, set)):
			return sorted(str(k) for k in keys)
	if isinstance(history_keys, (list, tuple, set)):
		return sorted(str(k) for k in history_keys)
	return []


def _history_key_counts(history_keys):
	...
	for name, meta in keys.items():
		if not isinstance(meta, dict):
			continue
		type_counts = meta.get("typeCounts") or []
		counts[str(name)] = sum(
...
def collect_runs():
	...
	for run in runs:
		...
		history_key_names = _history_key_names(history_keys)
		records.append(
			{
				"id": run.id,
				"name": run.name,
				...
				"history_key_count": len(history_key_names),
				"history_keys": history_key_names,
				"history_key_counts": _history_key_counts(history_keys),
